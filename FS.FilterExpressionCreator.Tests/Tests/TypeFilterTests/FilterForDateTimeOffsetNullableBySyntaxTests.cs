...
{
    [TestClass]
    public class FilterForDateTimeOffsetNullableBySyntaxTests : TestBase<DateTimeOffset?>
    {
        [DataTestMethod]
        [FilterTestDataSource(nameof(_testCases), nameof(TestModelFilterFunctions))]
        public void FilterForDateTimeNullableBySyntax_WorksAsExpected(FilterTestCase<DateTimeOffset?, DateTimeOffset?> testCase, TestModelFilterFunc<DateTimeOffset?> filterFunc)
        ...
        private static readonly FilterTestCase<DateTimeOffset?, DateTimeOffset?>[] _testCases = {
            ...
            FilterTestCase.Create<DateTimeOffset?>(1002, "~2010+02:00", x => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(2)) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.FromHours(2))),
            FilterTestCase.Create<DateTimeOffset?>(1003, "~2010", x => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.Zero)),
            ...
            FilterTestCase.Create<DateTimeOffset?>(1015, "2010-13-40", new FilterExpressionCreationException("Unable to parse given filter value")),
            FilterTestCase.Create<DateTimeOffset?>(1016, "2010-13-40_2020-01-01", new FilterExpressionCreationException("Unable to parse given filter value")),
            FilterTestCase.Create<DateTimeOffset?>(1017, "INVALID_june-16th", new FilterExpressionCreationException("Unable to parse given filter value")),
            FilterTestCase.Create<DateTimeOffset?>(1018, "june 1st_INVALID", new FilterExpressionCreationException("Unable to parse given filter value")),
            ...
            FilterTestCase.Create<DateTimeOffset?>(1102, "2010-01-01+02:00", x => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(2)) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.FromHours(2))),
            FilterTestCase.Create<DateTimeOffset?>(1103, "2010-01-01", x => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.Zero)),
            FilterTestCase.Create<DateTimeOffset?>(1104, "2010-01-01-02:00", x => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(-2)) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.FromHours(-2))),
            FilterTestCase.Create<DateTimeOffset?>(1105, "2010-06-01", x => x >= new DateTimeOffset(2010, 06, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2010, 07, 01, 0, 0, 0, TimeSpan.Zero)),
            FilterTestCase.Create<DateTimeOffset?>(1106, "2010-06-15", x => x >= new DateTimeOffset(2010, 06, 15, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 16, 0, 0, 0, TimeSpan.Zero)),
            FilterTestCase.Create<DateTimeOffset?>(1107, "2010-06-15T12:00:00", x => x >= new DateTimeOffset(2010, 06, 15, 12, 00, 00, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 13, 00, 00, TimeSpan.Zero)),
            FilterTestCase.Create<DateTimeOffset?>(1108, "2010-06-15T12:30:00", x => x >= new DateTimeOffset(2010, 06, 15, 12, 30, 00, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 12, 31, 00, TimeSpan.Zero)),
            FilterTestCase.Create<DateTimeOffset?>(1109, "2010-06-15T12:30:30", x => x >= new DateTimeOffset(2010, 06, 15, 12, 30, 30, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 12, 30, 31, TimeSpan.Zero)),
            FilterTestCase.Create<DateTimeOffset?>(1110, "2100-01-01", _ => NONE),
            FilterTestCase.Create<DateTimeOffset?>(1111, "2010-06-01_2010-06-15T12:31:00", x => x >= new DateTimeOffset(2010, 06, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 12, 31, 00, TimeSpan.Zero)),
            ...
            FilterTestCase.Create<DateTimeOffset?>(1202, "~2010-01-01+02:00", x => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(2)) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.FromHours(2))),
            FilterTestCase.Create<DateTimeOffset?>(1203, "~2010-01-01", x => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.Zero)),
            FilterTestCase.Create<DateTimeOffset?>(1204, "~2010-01-01-02:00", x => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(-2)) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.FromHours(-2))),
            FilterTestCase.Create<DateTimeOffset?>(1205, "~2010-06-01", x => x >= new DateTimeOffset(2010, 06, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2010, 07, 01, 0, 0, 0, TimeSpan.Zero)),
            FilterTestCase.Create<DateTimeOffset?>(1206, "~2010-06-15", x => x >= new DateTimeOffset(2010, 06, 15, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 16, 0, 0, 0, TimeSpan.Zero)),
            FilterTestCase.Create<DateTimeOffset?>(1207, "~2010-06-15T12:00:00", x => x >= new DateTimeOffset(2010, 06, 15, 12, 00, 00, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 13, 00, 00, TimeSpan.Zero)),
            FilterTestCase.Create<DateTimeOffset?>(1208, "~2010-06-15T12:30:00", x => x >= new DateTimeOffset(2010, 06, 15, 12, 30, 00, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 12, 31, 00, TimeSpan.Zero)),
            FilterTestCase.Create<DateTimeOffset?>(1209, "~2010-06-15T12:30:30", x => x >= new DateTimeOffset(2010, 06, 15, 12, 30, 30, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 12, 30, 31, TimeSpan.Zero)),
            FilterTestCase.Create<DateTimeOffset?>(1210, "~2100-01-01", _ => NONE),
            FilterTestCase.Create<DateTimeOffset?>(1211, "2010-06-01_2010-06-15T12:31:00", x => x >= new DateTimeOffset(2010, 06, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 12, 31, 00, TimeSpan.Zero)),
