...
namespace FS.FilterExpressionCreator.Tests.Tests.TypeFilterTests
{
    ...
    public class FilterForDateTimeOffsetByValueTests : TestBase<DateTimeOffset>
    {
        ...
        public void FilterForDateTimeByValue_WorksAsExpected(object testCase, TestModelFilterFunc<DateTimeOffset> filterFunc)
        {
            switch (testCase)
            {
        ...
        private static readonly object[] _testCases = {
            FilterTestCase.Create(1100, FilterOperator.Default, new DateTimeOffset[] { new (2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(2)) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(2)) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.FromHours(2))),
            FilterTestCase.Create(1101, FilterOperator.Default, new DateTimeOffset[] { new (2010, 01, 01, 0, 0, 0, TimeSpan.Zero) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.Zero)),
            FilterTestCase.Create(1102, FilterOperator.Default, new DateTimeOffset[] { new (2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(-2)) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(-2)) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.FromHours(-2))),
            FilterTestCase.Create(1103, FilterOperator.Default, new DateTimeOffset[] { new (2010, 06, 01, 0, 0, 0, TimeSpan.Zero) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 06, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2010, 07, 01, 0, 0, 0, TimeSpan.Zero)),
            FilterTestCase.Create(1104, FilterOperator.Default, new DateTimeOffset[] { new (2010, 06, 15, 0, 0, 0, TimeSpan.Zero) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 06, 15, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 16, 0, 0, 0, TimeSpan.Zero)),
            FilterTestCase.Create(1105, FilterOperator.Default, new DateTimeOffset[] { new (2010, 06, 15, 12, 00, 00, TimeSpan.Zero) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 06, 15, 12, 00, 00, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 13, 00, 00, TimeSpan.Zero)),
            FilterTestCase.Create(1106, FilterOperator.Default, new DateTimeOffset[] { new (2010, 06, 15, 12, 30, 00, TimeSpan.Zero) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 06, 15, 12, 30, 00, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 12, 31, 00, TimeSpan.Zero)),
            FilterTestCase.Create(1107, FilterOperator.Default, new DateTimeOffset[] { new (2010, 06, 15, 12, 30, 30, TimeSpan.Zero) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 06, 15, 12, 30, 30, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 12, 30, 31, TimeSpan.Zero)),
            FilterTestCase.Create(1108, FilterOperator.Default, new DateTimeOffset[] { new (2100, 01, 01, 0, 0, 0, TimeSpan.Zero) }, (DateTimeOffset _) => NONE),
            FilterTestCase.Create(1109, FilterOperator.Default, new DateTimeSpan[] { new (new DateTimeOffset(2010, 06, 01, 0, 0, 0, TimeSpan.Zero), new DateTimeOffset(2010, 06, 15, 12, 31, 00, TimeSpan.Zero)) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 06, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 12, 31, 00, TimeSpan.Zero)),

            FilterTestCase.Create(1200, FilterOperator.Contains, new DateTimeOffset[] { new (2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(2)) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(2)) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.FromHours(2))),
            FilterTestCase.Create(1201, FilterOperator.Contains, new DateTimeOffset[] { new (2010, 01, 01, 0, 0, 0, TimeSpan.Zero) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.Zero)),
            FilterTestCase.Create(1202, FilterOperator.Contains, new DateTimeOffset[] { new (2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(-2)) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 01, 01, 0, 0, 0, TimeSpan.FromHours(-2)) && x < new DateTimeOffset(2011, 01, 01, 0, 0, 0, TimeSpan.FromHours(-2))),
            FilterTestCase.Create(1203, FilterOperator.Contains, new DateTimeOffset[] { new (2010, 06, 01, 0, 0, 0, TimeSpan.Zero) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 06, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2010, 07, 01, 0, 0, 0, TimeSpan.Zero)),
            FilterTestCase.Create(1204, FilterOperator.Contains, new DateTimeOffset[] { new (2010, 06, 15, 0, 0, 0, TimeSpan.Zero) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 06, 15, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 16, 0, 0, 0, TimeSpan.Zero)),
            FilterTestCase.Create(1205, FilterOperator.Contains, new DateTimeOffset[] { new (2010, 06, 15, 12, 00, 00, TimeSpan.Zero) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 06, 15, 12, 00, 00, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 13, 00, 00, TimeSpan.Zero)),
            FilterTestCase.Create(1206, FilterOperator.Contains, new DateTimeOffset[] { new (2010, 06, 15, 12, 30, 00, TimeSpan.Zero) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 06, 15, 12, 30, 00, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 12, 31, 00, TimeSpan.Zero)),
            FilterTestCase.Create(1207, FilterOperator.Contains, new DateTimeOffset[] { new (2010, 06, 15, 12, 30, 30, TimeSpan.Zero) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 06, 15, 12, 30, 30, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 12, 30, 31, TimeSpan.Zero)),
            FilterTestCase.Create(1208, FilterOperator.Contains, new DateTimeOffset[] { new (2100, 01, 01, 0, 0, 0, TimeSpan.Zero) }, (DateTimeOffset _) => NONE),
            FilterTestCase.Create(1209, FilterOperator.Contains, new DateTimeSpan[] { new(new DateTimeOffset(2010, 06, 01, 0, 0, 0, TimeSpan.Zero), new DateTimeOffset(2010, 06, 15, 12, 31, 00, TimeSpan.Zero)) }, (DateTimeOffset x) => x >= new DateTimeOffset(2010, 06, 01, 0, 0, 0, TimeSpan.Zero) && x < new DateTimeOffset(2010, 06, 15, 12, 31, 00, TimeSpan.Zero)),
            ...
            FilterTestCase.Create(2000, FilterOperator.IsNull, (DateTimeOffset[])null, new FilterExpressionCreationException("Filter operator 'IsNull' not allowed for property type 'System.DateTimeOffset'")),

            FilterTestCase.Create(2100, FilterOperator.NotNull, (DateTimeOffset[])null, new FilterExpressionCreationException("Filter operator 'NotNull' not allowed for property type 'System.DateTimeOffset'")),
        };
