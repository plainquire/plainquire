...
[TestFixture]
public class FilterForDateOnlyNullableBySyntaxTests : TestContainer
{
    [FilterTestDataSource(nameof(_testCases))]
    public void FilterForDateOnlyNullableBySyntax_WorksAsExpected(FilterTestCase<DateOnly?, DateOnly?> testCase, EntityFilterFunc<TestModel<DateOnly?>> filterFunc)
    ...
    [
        ...
        FilterTestCase.Create<DateOnly?>(2000, "ISNULL", x => x == null),

        FilterTestCase.Create<DateOnly?>(2100, "NOTNULL", x => x != null)
    ];
}