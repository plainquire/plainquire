...
[TestFixture]
public class FilterForDateOnlyByValueTests : TestContainer
{
    [FilterTestDataSource(nameof(_testCases))]
    public void FilterForDateOnlyByValue_WorksAsExpected(object testCase, EntityFilterFunc<TestModel<DateOnly>> filterFunc)
    {
        switch (testCase)
        {
    ...
    [
        ...
        FilterTestCase.Create(2000, FilterOperator.IsNull, new DateOnly[] { default }, new FilterExpressionException("Filter operator 'IsNull' not allowed for property type 'System.DateOnly'")),

        FilterTestCase.Create(2100, FilterOperator.NotNull, new DateOnly[] { default }, new FilterExpressionException("Filter operator 'NotNull' not allowed for property type 'System.DateOnly'"))
    ];
